...
export function ContactPage() {
  ...
  const supportEmail = 'support@usewpa.tech';
  const enterpriseMailto = `mailto:${supportEmail}?subject=WebPageAnalyz%20Enterprise%20inquiry`;
  return <PublicFrame kind="contact"><section className="aux-public__body aux-contact"><PageHeader index="03 / SUPPORT" eyebrow={enterprise ? 'ENTERPRISE SALES' : 'PUBLIC SUPPORT'} title={enterprise ? 'Start the Enterprise conversation.' : 'WebPageAnalyz Support'} detail={enterprise ? 'Enterprise is an invitation plan. This public handoff preserves the plan context without starting a charge or pretending that a human review is automatic.' : 'For account, billing, scan or technical questions, use the support address below.'} /><div className="aux-contact__grid"><article><Mail /><span>{enterprise ? 'ENTERPRISE INQUIRY' : 'SUPPORT EMAIL'}</span><h2><a className="aux-contact__email" href={enterprise ? enterpriseMailto : `mailto:${supportEmail}`}>{supportEmail}</a></h2><p>{enterprise ? 'Describe the organization, expected website volume and the capabilities you need.' : 'Tell us what you expected and what happened. Do not include passwords, API keys, authentication codes or other secrets.'}</p><a href={enterprise ? enterpriseMailto : `mailto:${supportEmail}`} className="aux-dark-button">{enterprise ? 'Email Enterprise sales' : 'Email support'} <ArrowRight /></a></article><article><LifeBuoy /><span>AUTHENTICATED SUPPORT</span><h2>Keep account context with the case.</h2><p>Sign in when a question needs a target, report reference or reply history attached to the correct workspace.</p><PortalButtonLink href="/login" className="aux-light-button">Sign in to support <ArrowRight /></PortalButtonLink></article></div><div className="aux-contact__note"><CircleHelp /><span>{enterprise ? 'This email does not create an account, activate Enterprise, or trigger a charge.' : 'The authenticated support center remains separate; this public page does not create a ticket or claim a response time.'}</span></div></section></PublicFrame>;
}
