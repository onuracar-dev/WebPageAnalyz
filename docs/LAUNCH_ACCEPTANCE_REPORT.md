...
- Billing: `BILLING_PROVIDER=paddle`; `ENTERPRISE_SALES_MODE=contact`; `PADDLE_ENVIRONMENT`; `PADDLE_API_KEY`; `PADDLE_WEBHOOK_SECRET`; `PADDLE_PRICE_SIGNAL`; `PADDLE_PRICE_STUDIO`. Enterprise price remains empty in contact mode. Stripe variables remain empty unless the non-launch legacy adapter is deliberately enabled.
